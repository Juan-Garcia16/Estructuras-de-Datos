...
struct graph *ReadGraph(int vertices, int edges, struct student temp[])
{
    ...
    for(idEdge = 1; idEdge <= G->m_edges; idEdge++)
    {
        u = 0;
        v = 0;
        scanf("%s %s", uStudent, vStudent); //de donde sale (u) a donde llega (v)
        for (int k = 1; k <= vertices; k++)
        {
            if (strcmp(uStudent, temp[k].name) == 0)
                u = k;
            if (strcmp(vStudent, temp[k].name) == 0)
                v = k;   
            if (u != 0 && v != 0)
                break;    
        }
        //scanf("%s %s", &u, &v); //de donde sale (u) a donde llega (v)
...
void DFS_Visit(struct graph *G, int u, int *time, int color[], int pi[], int d[], int f[], struct student temp[], int*idxMax)
{
    struct edge *tempEdge; //para recorrer la lista de adyasencia
    int v;
    if(temp[*idxMax].maxH < temp[u].maxH) 
        *idxMax=u;
    color[u] = GRAY;
    *time = *time + 1;
    d[u] = *time;
    ...
    while (tempEdge != NULL)
    {
        ...
        {
            pi[v] = u;
            DFS_Visit(G, v, &(*time), color, pi, d, f, temp, idxMax);
        }
        tempEdge = tempEdge->next;
    }
    color[u] = BLACK;
...
void DFS(struct graph *G, int color[], int pi[], int d[], int f[], struct student temp[], int i)
{
    ...
    for (u = 1; u <= G->n_vertex; u++)
    {
        if (color[u] == WHITE) {
            idxMax = u;
            DFS_Visit(G, u, &time, color, pi, d, f, temp, &idxMax);
            printf("%s\n", temp[idxMax].name);
